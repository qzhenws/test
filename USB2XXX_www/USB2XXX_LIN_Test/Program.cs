    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                string path = System.Windows.Forms.Application.StartupPath;//获取文件目录
                DirectoryInfo dir = new DirectoryInfo(path);
                foreach (FileInfo f in dir.GetFiles("*.HEX"))//在程序文件目录中判断hex文件是否存在
                {
                    Application.Run(new Form1());//有hex文件时调用简易烧入程序
                    return;
                }
            ...
            {
                MessageBox.Show("未处理的异常！" + e.ToString());
                Application.Exit();
            }
            ...
            {
                MessageBox.Show("线程异常！" + e.ToString());
                Application.Exit();
            }
